...
internals.Connection.prototype.start = function (callback) {
    ...
    Pg.connect(internals.connectionConfig, function(err, pgClient, done) {
        ...
        pgClient.query('SHOW server_version;', function(err, result) {
            ...
            that.clientPoolIsReady = !!Pg.pools.all[key];
...
internals.Connection.prototype.validateSegmentName = function (name) {
    ...
    if (name.length > 63) {
        return new Error('Postgres identifiers must be less than 64 characters (segment is a table)');
    }

    if (!/[a-zA-Z_]/.test(name.charAt(0))) {
        return new Error('Postgres identifiers must begin with a letter or an underscore (segment is a table)');
    }
